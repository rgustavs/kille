...
--
-- Identiskt innehåll med schema.sql — de 98 satserna körs här via EXECUTE
...
create extension if not exists pgcrypto$kille_s1$;
  execute $kille_s2$-- ─── Tabeller ────────────────────────────────────────────────────────────────
...
create table if not exists public.kille_groups (
  ...
  execute $kille_s3$-- Idempotent för databaser som skapades innan slug fanns.
alter table public.kille_groups add column if not exists slug text$kille_s3$;
  execute $kille_s4$create unique index if not exists kille_groups_slug_key on public.kille_groups (slug)$kille_s4$;
  execute $kille_s5$-- Globala administratörer (super-admin) som hanterar alla grupper och användare.
create table if not exists public.kille_admins (
  ...
  execute $kille_s6$create table if not exists public.kille_group_members (
  id         uuid primary key default gen_random_uuid(),
  ...
  execute $kille_s7$-- Unikt medlemsnamn per grupp (skiftlägesokänsligt). Uttrycket kräver ett
...
create unique index if not exists kille_group_members_group_name_key
  on public.kille_group_members (group_id, lower(name))$kille_s7$;
  execute $kille_s8$-- Senast aktiv-tidsstämpel per medlem (heartbeat) — driver "aktiva medlemmar".
-- Idempotent för databaser som skapades innan kolumnen fanns.
alter table public.kille_group_members add column if not exists last_seen_at timestamptz$kille_s8$;
  execute $kille_s9$create table if not exists public.kille_group_players (
  id         text not null,
  group_id   uuid not null references public.kille_groups(id) on delete cascade,
  name       text not null,
  created_at timestamptz not null default now(),
  ...
  execute $kille_s10$-- Personer: samma människa dyker ofta upp i flera grupper, och kan finnas både
...
create table if not exists public.kille_people (
  ...
  execute $kille_s11$-- Idempotent för databaser som skapades innan personer fanns.
alter table public.kille_group_players
  add column if not exists person_id uuid references public.kille_people(id) on delete set null$kille_s11$;
  execute $kille_s12$alter table public.kille_group_members
  add column if not exists person_id uuid references public.kille_people(id) on delete set null$kille_s12$;
  execute $kille_s13$create index if not exists kille_group_players_person on public.kille_group_players (person_id)$kille_s13$;
  execute $kille_s14$create index if not exists kille_group_members_person on public.kille_group_members (person_id)$kille_s14$;
  execute $kille_s15$create table if not exists public.kille_group_games (
  id         text not null,
  group_id   uuid not null references public.kille_groups(id) on delete cascade,
  data       jsonb not null,
  ...
  execute $kille_s16$-- Turneringar: deltagare, omgångar och bord. Varje bord pekar på ett spel i
...
create table if not exists public.kille_group_tournaments (
  ...
  execute $kille_s17$-- Användnings-/aktivitetslogg (append-only). Fylls dels automatiskt inifrån
...
create table if not exists public.kille_activity (
  ...
  execute $kille_s18$create index if not exists kille_activity_group_time on public.kille_activity (group_id, created_at desc)$kille_s18$;
  execute $kille_s19$create index if not exists kille_activity_time       on public.kille_activity (created_at desc)$kille_s19$;
  execute $kille_s20$create index if not exists kille_activity_type_time  on public.kille_activity (event_type, created_at desc)$kille_s20$;
  execute $kille_s21$-- ─── Row Level Security (deny-all för anon; åtkomst via RPC nedan) ────────────

alter table public.kille_groups          enable row level security$kille_s21$;
  execute $kille_s22$alter table public.kille_group_members   enable row level security$kille_s22$;
  execute $kille_s23$alter table public.kille_group_players   enable row level security$kille_s23$;
  execute $kille_s24$alter table public.kille_group_games     enable row level security$kille_s24$;
  execute $kille_s25$alter table public.kille_group_tournaments enable row level security$kille_s25$;
  execute $kille_s26$alter table public.kille_admins          enable row level security$kille_s26$;
  execute $kille_s27$alter table public.kille_people          enable row level security$kille_s27$;
  execute $kille_s28$alter table public.kille_activity        enable row level security$kille_s28$;
  execute $kille_s29$revoke all on public.kille_groups        from anon, authenticated$kille_s29$;
  execute $kille_s30$revoke all on public.kille_group_members from anon, authenticated$kille_s30$;
  execute $kille_s31$revoke all on public.kille_group_players from anon, authenticated$kille_s31$;
  execute $kille_s32$revoke all on public.kille_group_games   from anon, authenticated$kille_s32$;
  execute $kille_s33$revoke all on public.kille_group_tournaments from anon, authenticated$kille_s33$;
  execute $kille_s34$revoke all on public.kille_admins        from anon, authenticated$kille_s34$;
  execute $kille_s35$revoke all on public.kille_people        from anon, authenticated$kille_s35$;
  execute $kille_s36$revoke all on public.kille_activity      from anon, authenticated$kille_s36$;
  execute $kille_s37$-- ═══════════════════════════════════════════════════════════════════════════
...
$$$kille_s37$;
  execute $kille_s38$-- Kräver giltig admin_code för gruppen, annars fel.
...
$$$kille_s38$;
  execute $kille_s39$-- Genererar en unik, läsbar join-kod (undviker lätt förväxlade tecken).
...
$$$kille_s39$;
  execute $kille_s40$-- Gör om ett gruppnamn till en URL-vänlig slug (t.ex. "gustavsson-and-friends").
...
$$$kille_s40$;
  execute $kille_s41$-- Skapar en unik slug utifrån ett namn (lägger till -2, -3 … vid krock).
...
$$$kille_s41$;
  execute $kille_s42$-- Bygger en komplett ögonblicksbild av en grupp (för login/pull).
...
$$$kille_s42$;
  execute $kille_s43$-- Upsertar en medlem (unikt namn per grupp) och returnerar rollen.
...
$$$kille_s43$;
  execute $kille_s44$-- Normaliserad namnnyckel: hittar samma namn oavsett skiftläge, mellanslag och
...
as $$
  select regexp_replace(
           ...
           '[^a-z0-9]+', '', 'g');
$$$kille_s44$;
  execute $kille_s45$-- Spelarens statistik inom en grupp: antal spel hen deltagit i och antal
...
$$$kille_s45$;
  execute $kille_s46$-- ─── Aktivitetsloggning ────────────────────────────────────────────────────────
...
$$$kille_s46$;
  execute $kille_s47$-- Uppdaterar "senast aktiv" för en medlem (heartbeat). No-op om id saknas.
...
$$$kille_s48$;
  execute $kille_s49$-- Hämta en grupp via dess slug (används för URL-åtkomst /?g=slug).
...
$$$kille_s49$;
  execute $kille_s50$-- Logga in i en grupp med join-kod. Registrerar valfritt medlemsnamn.
...
$$$kille_s50$;
  execute $kille_s51$-- Hämta senaste ögonblicksbild av gruppen. p_member_id (valfritt) uppdaterar
...
drop function if exists public.kille_pull(uuid, text)$kille_s51$;
  execute $kille_s52$create or replace function public.kille_pull(
...
$$$kille_s52$;
  execute $kille_s53$-- Verifiera admin-kod (låser upp adminläge på enheten).
...
$$$kille_s53$;
  execute $kille_s54$-- Spara (upsert) en spelare i gruppens gemensamma roster.
-- p_member_id/p_member_name (valfria) anger vem som gjorde ändringen och används
-- för aktivitetsloggen samt "senast aktiv".
drop function if exists public.kille_save_player(uuid, text, text, text)$kille_s54$;
  execute $kille_s55$create or replace function public.kille_save_player(
...
$$$kille_s55$;
  execute $kille_s56$-- Ta bort en spelare från gruppens roster.
drop function if exists public.kille_delete_player(uuid, text, text)$kille_s56$;
  execute $kille_s57$create or replace function public.kille_delete_player(
...
$$$kille_s57$;
  execute $kille_s58$-- Spara (upsert) ett helt spel i den centrala databasen.
drop function if exists public.kille_save_game(uuid, text, jsonb)$kille_s58$;
  execute $kille_s59$create or replace function public.kille_save_game(
...
$$$kille_s59$;
  execute $kille_s60$-- Ta bort ett spel ur den centrala databasen.
drop function if exists public.kille_delete_game(uuid, text, text)$kille_s60$;
  execute $kille_s61$create or replace function public.kille_delete_game(
  p_group_id uuid, p_join_code text, p_id text,
  p_member_id uuid default null, p_member_name text default null
)
returns jsonb
...
$$$kille_s61$;
  execute $kille_s62$-- Spara (upsert) en hel turnering i den centrala databasen. Turneringens bord
-- pekar på vanliga spel, som sparas för sig via kille_save_game.
drop function if exists public.kille_save_tournament(uuid, text, jsonb)$kille_s62$;
  execute $kille_s63$create or replace function public.kille_save_tournament(
...
$$$kille_s63$;
  execute $kille_s64$-- Ta bort en turnering. Spelen den pekar på ligger kvar i historiken.
drop function if exists public.kille_delete_tournament(uuid, text, text)$kille_s64$;
  execute $kille_s65$create or replace function public.kille_delete_tournament(
  p_group_id uuid, p_join_code text, p_id text,
  p_member_id uuid default null, p_member_name text default null
)
returns jsonb
...
$$$kille_s65$;
  execute $kille_s66$-- Ta emot en batch klient-genererade produktanalys-händelser (skärmvisningar,
...
$$$kille_s66$;
  execute $kille_s67$-- Lämna gruppen (tar bort den egna medlemsposten).
...
$$$kille_s67$;
  execute $kille_s68$-- ─── Admin-åtgärder (kräver admin-kod) ───────────────────────────────────────
...
$$$kille_s68$;
  execute $kille_s69$create or replace function public.kille_admin_remove_member(
...
$$$kille_s69$;
  execute $kille_s70$-- Sätt (eller återkalla) admin-rollen för en medlem.
...
$$$kille_s70$;
  execute $kille_s71$-- Byt admin-kod.
...
$$$kille_s71$;
  execute $kille_s72$-- Generera en ny join-kod (ogiltigförklarar den gamla).
...
$$$kille_s72$;
  execute $kille_s73$-- Radera hela gruppen och all dess data.
...
$$$kille_s74$;
  execute $kille_s75$-- Finns någon super-admin uppsatt ännu? (publikt — avslöjar inget känsligt)
create or replace function public.kille_sa_exists()
returns boolean
language sql
security definer
set search_path = public, extensions
as $$ select exists (select 1 from public.kille_admins); $$$kille_s75$;
  execute $kille_s76$-- Skapa den första super-adminen (fungerar bara om ingen finns ännu).
...
$$$kille_s76$;
  execute $kille_s77$-- Logga in som super-admin.
...
$$$kille_s77$;
  execute $kille_s78$-- Lägg till ytterligare en super-admin.
...
$$$kille_s78$;
  execute $kille_s79$-- Lista alla grupper med räknare (för super-admin-konsolen).
...
$$$kille_s79$;
  execute $kille_s80$-- Super-admin: plattforms-övergripande användningsöversikt (KPI:er + tidsserie).
...
$$$kille_s80$;
  execute $kille_s81$-- Super-admin: senaste aktivitetsflöde (namngivet) över alla grupper.
...
$$$kille_s81$;
  execute $kille_s82$-- Skapa en grupp som super-admin.
...
$$$kille_s82$;
  execute $kille_s83$create or replace function public.kille_sa_rename_group(
...
$$$kille_s83$;
  execute $kille_s84$create or replace function public.kille_sa_set_slug(
...
$$$kille_s84$;
  execute $kille_s85$create or replace function public.kille_sa_regen_code(
...
$$$kille_s85$;
  execute $kille_s86$create or replace function public.kille_sa_delete_group(
  p_username text, p_password text, p_group_id uuid
)
returns jsonb
language plpgsql
...
$$$kille_s86$;
  execute $kille_s87$-- Lista medlemmar + spelare i en grupp (för användarhantering).
...
$$$kille_s87$;
  execute $kille_s88$create or replace function public.kille_sa_remove_member(
...
$$$kille_s88$;
  execute $kille_s89$create or replace function public.kille_sa_remove_player(
...
$$$kille_s89$;
  execute $kille_s90$-- ─── Titta i en grupp (läsläge) ──────────────────────────────────────────────
...
$$$kille_s90$;
  execute $kille_s91$-- ─── Användare över gruppgränserna ───────────────────────────────────────────
...
$$$kille_s91$;
  execute $kille_s92$-- Slå ihop identiteter till en person. `p_identities` är en array av
...
$$$kille_s92$;
  execute $kille_s93$-- Byt visningsnamn på en sammanslagen person.
...
$$$kille_s93$;
  execute $kille_s94$-- Dela upp en person igen: identiteterna blir fristående, inget annat ändras.
...
$$$kille_s94$;
  execute $kille_s95$-- Koppla loss EN identitet från sin person. Blir det färre än två identiteter
...
from public, anon, authenticated$kille_s96$;
  execute $kille_s97$grant execute on function
...
to anon, authenticated$kille_s97$;
  execute $kille_s98$grant execute on function
...
to anon, authenticated$kille_s98$;
end
$kille_upgrade$;
